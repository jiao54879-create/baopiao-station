  ...
  async upsertProduct(input: ProductAnalysisInput, analysis: Partial<DeepAnalysis>) {
    return await prisma.insuranceProduct.upsert({
      ...
      create: {
        ...
        highlightsSevere: JSON.stringify(analysis.highlights?.重症 || []),
        highlightsMild: JSON.stringify(analysis.highlights?.轻症 || []),
        highlightsWaiver: JSON.stringify(analysis.highlights?.豁免 || []),
        highlightsSpecial: JSON.stringify(analysis.highlights?.特色 || []),
        highlightsValue: JSON.stringify(analysis.highlights?.增值服务 || []),
