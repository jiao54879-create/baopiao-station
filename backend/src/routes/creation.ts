...
### 配图标记规则（影响配图生成质量）
- **高亮标记（**文字**）必须克制使用**：每段最多1-2个高亮点，整篇不超过5个
- 高亮只标记最核心的结论/数字/反常识点，不是所有重点都标
- 普通重点用emoji或加粗语气强调即可，不需要**标记
- 换色标记（*文字*）只用于警示/坑点关键词
- 下划线标记（__文字__）用于专业术语
- ❌ 错误示例：每行都有**高亮**，满屏黄色，重点不突出
- ✅ 正确示例：一段话只在最关键的结论处用一次**高亮**
`;
...
| 禁用词 | 安全替代 |
|--------|----------|
| 最好的保险、性价比最高 | 我整理的几款热门保险、适合大多数人的 |
| 全网第一、销量第一、TOP1 | 人气很高、很多人选择、口碑不错 |
| 最便宜、最划算 | 百元级保障、学生党友好、高性价比之选 |
| 有病治病、无病养老 | 保障+储蓄双重功能、身故返还保费 |
| 专家推荐、央视推荐 | 很多博主都在推、我自己也买了 |
| 无效退款、不满意退保 | 犹豫期内可全额退保、退保规则清晰 |
| 什么都保、全险、万能险 | 保障全面、覆盖多种风险、综合保障 |
| 比社保好、吊打XX | 社保的有力补充、各有优势 |

### 中风险词（需谨慎）
| 禁用词 | 安全替代 |
|--------|----------|
| 最适合、必买、一定要买 | 建议配置、适合XX人群、我认为值得买 |
| 免费、白嫖、零元购 | 免费咨询、免费测算、免费方案定制 |
| 躺着赚钱、被动收入、财务自由 | 被动现金流、养老补充、财富积累 |
| 确诊即赔、立刻赔钱 | 符合条件即可赔付、理赔流程简单 |
| 不用体检、免健康告知 | 健康告知宽松、智能核保便捷 |
| 收益最高、回报率最高 | 收益稳定、复利增长、长期收益可观 |

### 绝对不能做的事
1. ❌ 留联系方式（微信/电话/二维码/私信领资料）→ 判定引流违规
2. ❌ 贬低其他保险公司（"XX垃圾""XX不好"）→ 限流
3. ❌ 承诺具体收益（"每年3.5%"）→ 只能说"现金价值写入合同"
4. ❌ 分享理赔案例时推荐产品 → 可以分享经历但不能带货
5. ❌ 使用谐音/拼音/符号规避（"zui""di一""抗ai"）→ 系统精准识别
6. ❌ "免费送保险"等诱导话术 → 虚假宣传
7. ❌ 绝对化用语：最佳/最优/最好/最高/最低/顶级/极致/完美/100%有效/万能/永久/独家/王牌/天花板
8. ❌ 医疗用语：治疗/治愈/根治/药到病除/特效（非医疗资质账号）

### 推流友好要素（增加流量）
1. ✅ 标题和正文前100字布局搜索关键词
2. ✅ 真实经历+客观分析，不用夸张表述
3. ✅ 具体数字代替模糊描述（"30岁男年交3800"而非"很便宜"）
4. ✅ 互动引导：提问/投票/评论区讨论
5. ✅ 完整内容不发半截，增加停留时长`;
...
15. 【配图友好】高亮标记(**...**)必须克制，每段最多1-2个，整篇不超过5个。高亮只标记最核心结论，不要满屏高亮
