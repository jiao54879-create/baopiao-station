/**
 * 同步产品数据到 Railway（更新真实价格）
 * 运行方式: npx tsx src/scripts/sync-products-railway.ts
 */
import pg from 'pg';
...
// 2026年主流重疾险产品数据（30岁男，50万保额，30年交，保终身）
const products = [
  {
    name: '达尔文12号',
    company: '复星联合健康保险',
    ...
    advantagesPrice: JSON.stringify(['50万保额年缴6680元（30岁男）', '终身多次赔价格最优']),
    ...
    source: '慧择保险网',
    sourceUrl: 'https://xuexi.huize.com/study/detal-547209.html',
  ...
  },
  {
    ...
    advantagesPrice: JSON.stringify(['50万保额年缴6850元（30岁男）', '综合性价比高（考虑医疗金后）']),
  ...
  },
  {
    ...
    source: '新浪财经',
    sourceUrl: 'https://cj.sina.com.cn/articles/view/7879922977/1d5ae152101901ccke',
  ...
  },
  {
    ...
    highlightsValue: JSON.stringify(['女性保费比男性低约6%', '30岁女性年缴6150元', '女性专属费率优势']),
    advantagesPrice: JSON.stringify(['30岁女性年缴6150元（女性最低）', '30岁男性年缴6550元', '女性保费优惠显著']),
  ...
  },
  {
    ...
    advantagesPrice: JSON.stringify(['性价比高，30岁男性年缴6580元起', '可选保至70岁降低保费']),
...
async function syncProductsToRailway() {
  ...
  try {
    ...
    console.log('\n✨ 产品数据同步完成！');
    console.log('\n📊 价格参考（30岁男，50万保额，30年交，保终身）：');
    products.forEach(p => {
      console.log(`   ${p.name}: ¥${p.priceAdult30}/年`);
    });
