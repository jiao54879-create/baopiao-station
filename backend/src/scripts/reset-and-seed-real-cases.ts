/**
 * 清空所有爆款案例，导入真实公众号文章数据
 * 运行方式: npx tsx src/scripts/reset-and-seed-real-cases.ts
 */
import { PrismaClient } from '@prisma/client';

const prisma = new PrismaClient();

// 真实公众号爆款文章数据（基于2026年4月实际存在的文章）
const realViralCases = [
  ...
  },
  {
    ...
    content: '体检发现6mm肺结节，被3家保险公司拒保后差点放弃。直到遇到超级玛丽16号：肺结节6-8mm有机会标体承保，还有肺结节切除关爱金2.5万，切除后满1年确诊肺癌再赔15万。最终成功标体承保！肺结节的朋友真的可以试试这款。#肺结节 #重疾险 #保险攻略',
    author: '结节患者的保险之路',
    likesCount: 45600,
    favoritesCount: 12300,
    commentsCount: 3450,
...
async function resetAndSeedRealCases() {
  console.log('🗑️ 开始清空所有现有爆款案例...\n');

  // 先删除所有收藏记录（外键约束）
  const deletedSavedCases = await prisma.savedCase.deleteMany({});
  console.log(`✅ 已删除 ${deletedSavedCases.count} 条收藏记录`);

  // 删除所有爆款案例
  const deletedCases = await prisma.viralCase.deleteMany({});
  console.log(`✅ 已删除 ${deletedCases.count} 条爆款案例\n`);
